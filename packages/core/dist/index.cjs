 ...
 */
function areDefinitionsEqual(left, right, seen = new WeakMap()) {
    if (left === right)
        return true;
    ...
    return (leftKeys.length === rightKeys.length &&
        leftKeys.every((key) => Object.prototype.hasOwnProperty.call(right, key) && areDefinitionsEqual(left[key], right[key], seen)));
}
/**
