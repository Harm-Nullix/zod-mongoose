...
Use `InferDocument` for values returned by Mongoose and `InferInput` for data accepted by Zod. The document type includes Mongoose's generated `_id`; the input type retains Zod's input types for coercions and preprocessors.
...
type User = InferDocument<typeof UserSchema>;
type CreateUser = InferInput<typeof UserSchema>;
...
`setFrontendMode()` is deprecated in v3.1 and logs a warning when called. Remove it and rely on conditional exports instead.

```typescript
import { setFrontendMode } from '@nullix/zod-mongoose';

// Manually enable frontend mode
setFrontendMode(true); // Deprecated: remove this call
```

## Using with Nuxt 4 & Nitro

`zod-mongoose` is optimized for Nuxt 4. Import shared schema helpers from `@nullix/zod-mongoose/nuxt`; Nuxt then selects the browser-safe client entry or the Nitro server entry automatically. The root import remains supported in v3.1 but is deprecated for Nuxt code and will be removed in v4.
