...
### Explicit Subschemas
By default, nested Zod objects are mapped to nested documents (nested paths) in Mongoose. If you want a nested object to be a separate Mongoose subschema (with its own `_id`, middleware, etc.), use the `schema` property.
...
You can also pass `SchemaOptions` to the `schema` property:
