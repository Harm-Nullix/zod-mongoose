...
For a top-level discriminated union, `toMongooseSchema()` also requires either `modelName` or `collection`. If only `collection` is provided, the base model name is derived by singularizing the collection name. If neither is provided, the function throws an explanatory error because Mongoose registers discriminator model names globally.
...
See [Nested Object IDs](/api/nested-object-ids) for a runnable comparison of these options, and [Reusing Object Schemas](/api/reusing-object-schemas) for imports shared across files and arrays.
