...
| `z.object()` | `Subdocument` | Mapped to a Mongoose subschema (subdocument) with its own `_id` by default. Use `withMongoose(schema, { schema: { _id: false } })` to keep the subschema without its ID, or `{ schema: false }` for nested paths. See [Nested Object IDs](/api/nested-object-ids). |
