...
| `z.object()` | `Nested Document` | Mapped to a nested object (nested paths) by default. Use `withMongoose(schema, { schema: true })` to create a subschema (subdocument). |
| `z.intersection()` | `Nested Document` | Merges the definitions of both branches into a single nested object. |
