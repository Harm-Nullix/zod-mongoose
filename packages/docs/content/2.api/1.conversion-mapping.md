...
| `z.object()` | `Subdocument` | Mapped to a Mongoose subschema (subdocument) by default. Use `withMongoose(schema, { schema: false })` to map to a nested object (nested paths). |
