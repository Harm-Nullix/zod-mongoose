...
An alias for `OutputMongoose<T>`. Deprecated in v3.1; use `InferDocument<T>` instead. It remains available until v4.
...
Infers the input type for a Zod schema, equivalent to `z.input<T>`. Deprecated in v3.1; use `InferInput<T>` instead.
