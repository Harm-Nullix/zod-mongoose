...
Flat options on a top-level `withMongoose` call apply to the root schema. They do not turn off IDs for nested objects. For options on an imported object that you use in several places, see [Reusing Object Schemas](/guides/reusing-object-schemas).
